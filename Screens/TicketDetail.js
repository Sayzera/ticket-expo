...
const TicketDetail = () => {
  ...
  const date = new Date(params.ticket_date.seconds * 1000);
  ...
  const getTicketImages = async () => {
    ...
    const storageRef = ref(storage, `tickets/${params.user_id}/${params.id}`);
  ...
  const getUserData = async () => {
    ...
    const q = query(userRef, where('user_id', '==', auth.currentUser.uid));
  ...
  const answerTicket = async (data) => {
    ...
    const docRef = addDoc(collection(db, 'ticketsAnswers'), {
      answer: data.answer,
      ticket_id: params.id,
  ...
  const getTicketAnswers = async () => {
    ...
    setTicketAnswers(
      querySnapshot.docs.map((doc) => {
        if (doc.data().ticket_id === params.id) {
          return {
            id: doc.id,
            ...doc.data(),
          };
  ...
  return (
    <SafeAreaView className="flex flex-col flex-1  bg-white">
      <Back />
      <Header title={'Customer: ' + params.name}>
